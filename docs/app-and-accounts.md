...
- **Starred jobs.** A ★ on every result. It changes nothing about the search — a starred job ranks exactly where it always did, with the star filled in so the list tells you which ones are yours — and everything starred is listed under Saved. The star fills in the instant it is pressed and the request goes out behind it; if the server refuses, the star goes back and the page says so. Signed out it goes to the sign-in screen rather than disappearing, because a control that vanishes teaches nobody what an account is for.
- **Applied jobs.** A ✓ between the star and the ×, for the decision neither of them covers: the job you just finished applying to is not a favourite and not a rejection. Pressing it files the job under Applied with the date on it *and* takes it out of every later search, because a board that keeps offering back the job you already applied to is wrong in the same way as one that keeps offering the job you turned down. The row leaves the list at once, leaving a line naming it and an Undo; pressing the ✓ again on a job already marked puts it back in your results. Nothing is thrown away — the job is kept, which is the difference between this and the ×.
- **Hidden jobs.** A × beside the star. Pressing it takes the row off the list at once — before the server has heard about it, the same way the star works — leaving a line naming the job and an Undo, and from the next search on that posting is gone: the account hands the engine a set of ids and the engine subtracts them before it counts anything ([filtering.md](./filtering.md) covers why that is a set of ids and not a criterion). The Hidden tab in the saved view lists them and brings any of them back.
- **What was done about them.** Each saved job carries a status — saved / applied / interviewing / offer / rejected — and a private note. Every status from `applied` on holds the job out of your searches, which is the ✓'s rule said the long way: moving a job on to interviewing is not a request to see the posting again tomorrow, and rejected least of all. `saved` is the only status that leaves a job in your results, which is the star's whole promise. The page draws the ✓ from a `hides` flag served with the status vocabulary rather than from a second list of its own, so the button and the engine cannot come to disagree.
...
**A search that held something back still says so.** The results line counts the matches it left out — `1,973 matching · 1 hidden by you · 3 already applied to` — and each count links to the screen those jobs are on. It is the same rule the duplicate fold follows: a result set that quietly shrinks is indistinguishable from a filter that went wrong, and the counts are of jobs matching *these* filters, not of everything ever hidden or applied to. The two are counted apart, and subtracted from two sets rather than one, because "you already did this" and "you said no to this" are different sentences and they are answered on different tabs. Hiding and starring stay independent of each other, because being rejected from a job you applied to is exactly when you want it out of your results and still in your history.
