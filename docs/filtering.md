...
| `sectors` / `exclude_sectors` | what the **company** does, from `SECTORS` in `schema.mjs` — `fintech`, `financial-services`, `healthcare`, `developer-tools`, `aerospace-defense`, thirty in all. Read off each company's own postings by the enrich pass (`npm run enrich`, one model call per company) and stored on `companies.sector`; every job inherits it. Not the job's function. A company nobody has read, or the model would not commit on, is `unknown` and goes to the `sector` policy; `other` is unknown too unless asked for by name. The exclusion cannot fire on silence — "not finance" drops a company *read as* a bank, never one whose postings did not say. |
